...
document.getElementById('send-reset-link-btn').onclick = async () => {
    ...
    try {
        ...
        showToast(err.response?.data?.message || 'Failed to send link', 'error');
    } finally {
        const btn = document.getElementById('send-reset-link-btn');
        btn.innerHTML = '<i class="ph ph-paper-plane-right"></i> SEND LINK'; // Hardcoded restore
        btn.disabled = false;
    }
};

// Check for Token in URL (Google Login)
...
if (tokenInUrl) {
    ...
    localStorage.setItem('user', JSON.stringify({ username: userName, role: userRole }));

    // Clean URL
    window.history.replaceState({}, document.title, window.location.pathname);

    // Reload to init dashboard
    location.reload();
}

// Check for Reset Token in URL
const resetTokenInUrl = urlParams.get('resetToken');
...
document.getElementById('confirm-reset-password-btn').onclick = async () => {
    ...
    try {
        await axios.post(`${API_BASE}/auth/reset-password`, { token: resetTokenInUrl, newPassword });
        showToast('Password reset successfully! Please login.', 'success');
        closeModal('reset-password-modal');
        document.getElementById('show-login').click();
    } catch (err) {
        showToast(err.response?.data?.message || 'Reset failed/expired', 'error');
    }
};
